...
const rewriteRules = {
    Drawing: function (_sequence) {
	var sequence = _sequence.rewrite ();
	return `
	<html>
          <body>
            <svg width="1000" height="400">
              ${sequence.map (s => {return s.text}).join ('\n')}
            </svg>
	   </body>
	</html>
	    `;
	...
	var es = accumulate (_es.rewrite (), "Seq 0");
	var eAndEs = accumulate ([e, es], "Seq 1");
	...
	var svg = `<text x="0" y="0" fill="#000000" font-family="Helvetica" font-size="12px" text-anchor="middle">${result.text}
	</text>`;
	result.text = svg;
	return result;
    },
